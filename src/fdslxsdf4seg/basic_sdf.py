...
class Cone(SDFObject):
    def __init__(
        ...
        D, H, W = grid_size
        if radius is None:
            radius = random.uniform(min(D, H) * 0.10, min(D, H) * 0.300)
        if height is None:
...
class Octahedron(SDFObject):
    def __init__(self, grid_size, device, center=None, transform=False, size=None):
        ...
        D, H, W = grid_size
        if size is None:
            self.size = random.uniform(min(D, H, W) * 0.10, min(D, H, W) * 0.300)
        else:
            self.size = size
...
class Cylinder(_PrismBase):
    ...
    def __init__(
        ...
        if radius is None:
            radius = random.uniform(0.100 * perp_min, 0.300 * perp_min)
        self.radius = radius
...
class ConvexCylinder(_ConvexPrismBase):
    ...
    def __init__(
        ...
        perp_min = min([D, H, W][i] for i in range(3) if i != axis)
        if radius is None:
            radius = random.uniform(0.10 * perp_min, 0.300 * perp_min)
        self.radius = radius
