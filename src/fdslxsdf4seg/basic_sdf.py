...
class Sphere(SDFObject):
    def __init__(
        ...
        # ランダム化
        if radius is None:
            radius = random.uniform(min(D, H, W) * 0.15, min(D, H, W) * 0.40)
        # パラメータ設定
        self.radius = radius
...
class Torus(SDFObject):
    def __init__(
        ...
        D, H, W = grid_size
        if major_r is None:
            major_r = random.uniform(min(D, H) * 0.15, min(D, H) * 0.40)
        if minor_r is None:
...
class Cone(SDFObject):
    def __init__(
        ...
        D, H, W = grid_size
        if radius is None:
            radius = random.uniform(min(D, H) * 0.15, min(D, H) * 0.40)
        if height is None:
            height = random.uniform(W * 0.10, W * 0.50)
        self.radius = radius
        self.height = height
...
class Octahedron(SDFObject):
    def __init__(self, grid_size, device, center=None, transform=False, size=None):
        ...
        D, H, W = grid_size
        if size is None:
            self.size = random.uniform(min(D, H, W) * 0.15, min(D, H, W) * 0.40)
        else:
            self.size = size
